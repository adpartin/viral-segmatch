...
def test_classify_unknown_when_any_null():
    assert classify_pair_regime((None, 'H3N2', 2024), ('Human', 'H3N2', 2024)) == 'unknown_metadata_neg'
    assert classify_pair_regime(('Human', 'H3N2', 2024), ('Human', None, 2024)) == 'unknown_metadata_neg'
    assert classify_pair_regime(('Human', 'H3N2', None), ('Human', 'H3N2', 2024)) == 'unknown_metadata_neg'
...
def test_compute_match_count():
    ...
    assert compute_match_count(('A', None, 'C'), ('A', 'B', 'C')) is None
...
def test_count_available_unknown_cell():
    cell_known = ('Human', 'H3N2', '2021+')
    cell_unknown = ('Human', 'H3N2', None)
    cell_counts = {cell_known: 3, cell_unknown: 2}
    avail = count_available_per_regime(cell_counts)
    # Within-cell pairs of cell_known: 3*2 = 6 -> host_subtype_year
    # Within-cell pairs of cell_unknown: 2*1 = 2 -> unknown_metadata_neg
    # Cross-cell: 3*2 + 2*3 = 12 -> unknown_metadata_neg
    assert avail['host_subtype_year'] == 6
    assert avail['unknown_metadata_neg'] == 2 + 12
...
def test_resolve_regime_targets_round_correction():
    # 1000 negatives across 9 regimes; 0.30 host_subtype_year, 0.20 none_match,
    # 0.10 each for host_only/subtype_only/year_only/host_subtype_only,
    # 0.05 each for host_year_only/subtype_year_only, 0.00 unknown_metadata_neg.
    targets = {
        'none_match': 0.20,
        'host_only': 0.10,
        'subtype_only': 0.10,
    ...
    assert counts['unknown_metadata_neg'] == 0


def test_resolve_regime_targets_drift_correction():
    # Targets sum to 1 but rounding gives 99 or 101 for 100 budget.
    targets = {r: 1.0 / 9 for r in REGIME_NAMES}
...
if __name__ == '__main__':
    ...
    test_classify_unknown_when_any_null()
    ...
    test_count_available_unknown_cell()
