...
Key bundle parameters: `virus.selected_functions`, `dataset.max_isolates_to_process`, `dataset.hn_subtype`, `dataset.year`, `dataset.host`, `dataset.split_strategy.{mode,hash_key,cluster_alphabet,single_slot,negative_scope,m_pos_per_cc}`, `dataset.metadata_holdout`, `training.slot_transform`, `training.interaction`. Sklearn-baseline knobs live under `baseline_<name>.*` at the bundle root (defaults inherited from `conf/baselines/default.yaml`).
...
src/
  ...
  utils/
    ...
    clustering_utils.py             # mmseqs2 wrappers; alphabet={aa,nt_cds,nt_ctg} (from schema registry), algorithm={cluster,linclust}
...
- **Sequence hashes**: `prot_hash = md5(prot_seq)` (protein), `ctg_dna_hash = md5(ctg_dna_seq)` (contig DNA), `cds_dna_hash = md5(cds_dna_seq)` (CDS DNA). In pair tables: `prot_hash_a`/`prot_hash_b`, `ctg_dna_hash_a`/`ctg_dna_hash_b`, `cds_dna_hash_a`/`cds_dna_hash_b`. Per-alphabet column/file names come from one source of truth — the `SCHEMA` registry in `src/utils/schema.py` (alphabet ∈ {`aa`, `nt_cds`, `nt_ctg`}). Each hash is produced and persisted at its source stage: Stage 1 (`preprocess_flu.py`) writes `prot_hash` into `protein_final` and `ctg_dna_hash` into `ctg_dna_final`; Stage 1.5 (`extract_cds_dna.py`) writes `cds_dna_hash` into `cds_dna_final`. Stage 3 reads them (no recompute). ESM-2 cache key uses `sha1(prot_seq)` — separate namespace, never joined back to `prot_hash`.
...
- **aa/nt vs protein/DNA**: alphabet tokens `aa` / `nt_cds` / `nt_ctg` at the **alphabet/residue level** (kmer alphabet, cluster alphabet, "1 aa change", "97% nt identity"); molecule names `prot` / `cds_dna` / `ctg_dna` at the **molecule/sequence level** (sequence columns, hashes, file basenames). `nt` splits into `nt_cds` (CDS DNA) and `nt_ctg` (contig DNA); the pairing is aa↔protein, nt_cds↔CDS DNA, nt_ctg↔contig DNA — don't cross the streams. `src/utils/schema.py` is the registry that enforces this mapping.
- **atom = CC (2D-CD)**: in `cluster_disjoint_cc`, the routing atom is one bipartite CC (one atom per CC). `split_strategy.m_pos_per_cc` caps the positive **rows (pairs)** kept per CC — it does NOT change the atom count (atoms are fixed by the cluster threshold). Reserve "atom"/"CC" for components and "pair"/"row" for records. See `docs/methods/glossary.md` "Atom element" / "`m_pos_per_cc`".
