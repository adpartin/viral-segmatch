...
- **Proteins**: `preprocess_flu.py` maps GTO replicon functions to short protein names. `conf/virus/flu.yaml` knows 20 of them; the 8 selected for modelling, one per segment, are PB2, PB1, PA, HA, NP, NA, M1, NS1. The rest (M2, NEP, PA-X, PB1-F2, ...) are spliced, frameshifted or alternative-start products and are not modelled.
...
- **Sequence hashes**: `prot_hash = md5(prot_seq)`, `ctg_dna_hash = md5(ctg_dna_seq)`, `cds_dna_hash = md5(cds_dna_seq)`. In pair tables: `*_hash_a` / `*_hash_b`. Per-alphabet column/file names come from one source of truth — the `SCHEMA` registry in `src/utils/schema.py` (alphabet ∈ {`aa`, `nt_cds`, `nt_ctg`}). Each hash is produced/persisted at its source stage (Stage 1 writes `prot_hash`/`ctg_dna_hash`; Stage 1.5 writes `cds_dna_hash`); Stage 3 reads them (no recompute). ESM-2 cache key uses `sha1(esm2_ready_seq)` (the cleaned column, not `prot_seq`; set at `compute_esm2_embeddings.py:55`, hashed at `esm2_utils.py:643`) — separate namespace, never joined back to `prot_hash`.
...
- **Explanatory prose** (plan/results docs, and the WHY-text in docstrings and comments — not CLAUDE.md itself or other terse reference lists, which are for scanning, not narration): full sentences, one claim per sentence, not a bolded headline followed by a stat fragment. Prefer splitting into two sentences, or a `because`/`so`/`,` join, over an em-dash as a clause-joiner. State a claim's limits in their own sentence rather than leaving them implied (e.g. "this does not establish X", not silence). A fact repeated in more than one place uses the same phrasing every time it appears.
  Example — not: `**Per-site features work, at a third of the width.** F1 macro 0.9192 +/- 0.0134 for nt vs 0.9094 +/- 0.0145 for k-mer.` Instead: `The nucleotide-site model obtained an F1 macro of 0.9192 ± 0.0134. The k-mer model obtained 0.9094 ± 0.0145. With only 4 folds, this does not establish either model as superior.`
- **Plain words, and no narration after a doc edit**. Write the plainest sentence that carries the claim. Cut any sentence that only sets up the next one ("Three things stand in the way, and they call for different responses", "This is the case that matters here") — start with the claim instead. Prefer the shorter word: `affects` over `is the case that matters for`, `no remedy` over `nothing can be done about it`. After editing a doc, show the diff and stop. Do not recap what changed, why it changed, or what was left out unless asked; the diff already says it. The same applies to chat replies: answer what was asked and stop.
