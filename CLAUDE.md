...
- **Threshold notation**: `tXXX` (zero-padded, e.g., `t095`) denotes the mmseqs identity threshold (`--min-seq-id`) at `0.XXX` — in docs prose, tables, and (future) plot labels. Code/dirs/bundles still use `idXXX` (cluster output dirs at `data/processed/.../clusters_aa/id095/`, bundle filenames like `flu_*_id095_*.yaml`, run dirs `training_*_id095_*`, routing labels `cluster_disjoint_id099`). A full `idXXX → tXXX` migration across code/dirs/bundles is deferred until the next cluster-sweep regeneration (option B); current state is docs-only. Adopted 2026-05-29.
