...
- **Documentation language**: prefer plain technical verbs (`removes`, `drops`, `reads`, `writes`, `joins`) over decorative or colloquial alternatives (`scrubs`, `massages`, `munges`, `slurps`). The docs are technical references, not blog posts. Use the same word for the same action throughout a doc — don't synonym-vary for variety's sake.
