...
**Files**: `scripts/stage2_esm2.sh`

**DONE**: `stage1_preprocess_flu.sh` (54 lines), `stage2b_kmer.sh` (58 lines),
`stage3_dataset.sh` (80 lines), `stage4_train.sh` (125 lines) all follow the
lean pattern.

**Remaining**: `stage2_esm2.sh` (142 lines, verified 2026-05-12) still has the
verbose pattern (git provenance blocks, `log()` helpers, elaborate headers /
footers, registry integration). Should be refactored to match the lean pattern
used by the others.
