...
- **C1. Baseline reproduction** [DONE] — nt_cds clusters (t100) + nt_ctg features. Bundle `flu_ha_na_cluster_nt_cds_t100` (reconstructed from the surviving PB2-PB1 id100 `resolved_config`; HA-NA id100 dirs were deleted). **Reproduced:** LGBM test F1 **0.9569** (AUC-ROC 0.9920), MLP **0.9649** — vs the historical id100 LGBM 0.958, i.e. on-target to −0.1 pp. Run under the operationally-proper **nt_cds pair_key** (Option B, matches exp1); at strict t100 the pair_key change is negligible (the −1.9 pp seen at t099 doesn't apply). Confirms the refactor preserved the known result + the full Phase-C pipeline works end-to-end. (A `pair_key_alphabet=aa` historical-fidelity variant — nt_cds clusters + protein pair_key — is now runnable: the Phase-2 regression that left `cds_dna_hash` unpopulated was fixed in §13 step (c). Running it remains optional.)
- **C2. Exp 1** [DONE] — nt_cds clusters (t100) + nt_cds features (clean CDS). Bundle `flu_ha_na_cluster_nt_cds_t100_feat_nt_cds` (inherits C1's routing, overrides k-mer to nt_cds k6; reuses the C1 dataset via Stage 3/4 decoupling). **LGBM test F1 0.9532** (AUC-ROC 0.9904), **MLP 0.9625**. Isolates the FEATURE axis vs C1 (routing + pair_key fixed): nt_cds vs nt_ctg features are ~equal at t100 (LGBM 0.9532 vs C1 0.9569 −0.4 pp; MLP 0.9625 vs C1 0.9649 −0.2 pp) — the contig's flanking UTR adds ~nothing.
- **C3. Exp 2** [DONE] — nt_ctg clusters (t100) + nt_ctg features (clean contig). Bundle `flu_ha_na_cluster_nt_ctg_t100`. **LGBM test F1 0.9574** (AUC-ROC 0.9920), **MLP 0.9674**. Reported per option (2): "contig-as-submitted, UTR artifact included" — a sensitivity check (UTR-confound caveat §9/§12). pair_key defaults to **protein** (the nt_ctg inference), so vs C1 this differs on BOTH the cluster axis AND the pair_key — **not** a clean cluster-axis isolation. exp2 also carries 4–6% prot_hash (DNA-variant) leakage (distinct contigs → same protein across splits), which likely eases it (highest MLP, 0.9674). A pair_key-matched variant (nt_ctg clusters + nt_cds pair_key) would give a clean cluster-axis Δ vs C1 — deferred unless wanted.

**Phase C summary** (HA/NA, t100, cluster_disjoint): all three sit at ~0.95–0.97. LGBM C1 0.9569 / exp1 0.9532 / exp2 0.9574; MLP C1 0.9649 / exp1 0.9625 / exp2 0.9674. Feature axis (C1↔exp1) is flat (~0.3 pp; UTR adds ~nothing); cluster axis (C1↔exp2) is confounded by pair_key. The full nt_cds/nt_ctg pipeline is exercised end-to-end on all three.
...
**Scoping (decided 2026-06-25): the policy is OPT-IN via `dataset.molecule`.** A global guard would reject the existing fleet — `flu_ha_na`/`flu_pb2_pb1` (seq_disjoint) already run `nt_ctg` features + `aa` pair_key (an intentional, long-standing mismatch), and every existing bundle except exp1 is similarly mismatched. So enforcement applies only where opted in.

- **Master knob `dataset.molecule: aa|nt_cds|nt_ctg`** (optional). When set: derives `cluster_alphabet` + `pair_key_alphabet` **where absent** (an explicit per-axis value wins); `kmer.alphabet` is **authoritative** (always set to the molecule — to diverge features, use a legacy bundle). Resolved once in the shared loader (`config_hydra._resolve_molecule_alphabets`) so all stages (Stage 2b/3/4) see the same derived values.
- **Config-load guard (opt-in):** fires **only when `dataset.molecule` is set** — the derived axes must agree, **error** unless `dataset.allow_alphabet_mismatch: true`. **Legacy bundles (no molecule) are untouched** — the orchestrator's `:587–594` pair_key inference and per-stage `kmer.alphabet` defaults still apply, so the fleet is unaffected (no bundle changes needed).

Relation to **(c)**: the opt-in guard prevents mismatches *in molecule mode*. It does **not** auto-catch the §13.4 float64 crash in *legacy* mode (nt_cds cluster + non-nt_cds pair_key) — that remains the separate (c) item (robust populate-not-skip attach / clear error), to be handled after (a)+(b).
...
`v2:1575` gated the cds_dna_hash attach on column *presence*, but `build_pair_columns` always emits empty `cds_dna_hash_{a,b}`; with `pair_key≠nt_cds` they stayed float64-NaN → cluster join crashed on a float64-vs-string merge. Authored `577d19b5` (2026-06-02, Phase-2), **not** this refactor. **Resolution [DONE — step (c)]: populate-not-skip attach** — gate on *populated* (`.isna().all()`), not *present*, and drop the empty placeholders before reattaching. Applied to **both** v2 cluster-attach gates (holdout + CV). The historical nt_cds-cluster + protein-pair_key config now runs; the populated path (pair_key=nt_cds) is ε=0 unchanged.

### 13.5 Implementation order + regression

- **(a) nt_ctg pair_key enablement** [DONE — `76c4d32`] (7 sites + registry cleanup) — ships the *capability*; the nt_ctg-all-3 *run* stays deferred (§13.3). Regression delivered: contract suite + `test_cooccur_ternary_parity`; Stage-3 **ε=0** on the matched nt_cds dataset (byte-identical all splits); nt_ctg pair_key end-to-end (co-occurrence on `ctg_dna_hash`, pair_key 100% from `ctg_dna_hash` / 0% `prot_hash`). No model re-runs (ε=0).
- **(b) master knob (`dataset.molecule`) + opt-in guard** [DONE] — `config_hydra._resolve_molecule_alphabets` (derives absent cluster/pair_key, kmer authoritative, guard + `allow_alphabet_mismatch`). **No bundle changes required** (opt-in → legacy C1/exp2 untouched; the earlier "add allow flag to C1/exp2" assumed a global guard and no longer applies). Regression delivered: `tests/test_molecule_resolution.py` (7); legacy fleet verified no-op; demo bundle `flu_ha_na_molecule_nt_cds` derives all-nt_cds (guard passes); **ε=0** molecule-derived dataset == explicit C1 dataset (orchestrator consumes the derived `pair_key`).
- **(c) populate-not-skip attach** [DONE] — both v2 cluster-attach gates (holdout + CV) gate on *populated*, not *present* (§13.4). Validated: the previously-crashing nt_cds-cluster + protein-pair_key (Option-A) config builds end-to-end (attach fires, 0 dropped, pair_key 100% from prot_hash, 0 cluster leakage); ε=0 on the populated nt_cds-pair_key path (C1 unchanged).
- Order: (a) → (b) → (c). **All three done.** §13 axis-consistency work complete.

### 13.6 Resolved decisions (2026-06-25)

1. **Setting mechanism: master knob `dataset.molecule` + OPT-IN config-load guard.** The knob defaults cluster/pair_key (explicit overrides win) and is authoritative for kmer; the guard errors on mismatch unless overridden — **only when `dataset.molecule` is set** (scoping decided 2026-06-25, §13.2: a global guard would reject the intentionally-mismatched existing fleet).
...
4. **pair_key in molecule mode comes from the molecule or an explicit value** (never silently from `cluster_alphabet`). The `:587–594` inference is **retained for legacy bundles** (molecule mode bypasses it by writing pair_key explicitly) — the opt-in scoping means the inference can't be removed without reclassifying the fleet.
...
**Resolved during this work:** `cluster_disjoint_feasibility` CLI cutover (B4); the v2 `cds_dna_hash` attach-gate regression (§13c, populate-not-skip); `kmer_features_nt_ctg` metadata `alphabet` labels (integrity audit); surgery-parity of `protein_final`/`ctg_dna_final` verified against backups (audit).

**Remaining loose ends (non-blocking, carried to `.claude/memory.md`):** orchestrator (`dataset_segment_pairs.py`) still reads `cds_final` (works — has `cds_dna_hash` — but should be `cds_dna_final`); `compute_kmer_features` docstring labels (`nt:`→`nt_ctg:`); `train_pair_baselines` `kmer_alphabet` provenance (needs param threading); `compute_kmer_features` `INPUT_BASENAME` is `.csv` (resolves via `load_dataframe`, but ext-agnostic is cleaner); **`stage4_*.sh` wrappers call bare `python`** (not the segmatch env → `ModuleNotFoundError`; invoke trainers with the abs python or fix the wrappers); **post-v2 CC-builder convention pass** (`dataset_pairs_cc.py` → new seq/hash/cluster names + the `:375` aa-only pair_key gate); **deferred experiments** — UTR data-cleaning → clean nt_ctg-all-3 run (§13.3), nt_ctg feasibility below t099 via `drop_budget` mega-CC cut (§13.6/B4).

**Closed 2026-06-25 — IMPLEMENTED.** Contract suite at close: 27 tests (`test_schema` 11, `test_kmer_utils` 3, `test_clustering_utils` 6, `test_molecule_resolution` 7), run each file separately. HEAD `f619f98`. Remaining work is in the loose-ends above + `.claude/memory.md`. (This "checkpoint + resume" section is retained as the implementation log; it is no longer an active resume anchor.)
