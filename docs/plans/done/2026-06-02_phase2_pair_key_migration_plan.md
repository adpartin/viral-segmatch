...
6. **DONE 2026-06-03** — `data(models): retrain MLP + LGBM on test datasets; aa regression guard + nt cluster before/after measurement + routing triangulation`. 12 runs end-to-end (6 MLPs across 2 GPU chains + 6 LGBMs sequential on CPU; ~90 min MLP wall time, ~30 min LGBM). Results manifest at `docs/results/2026-06-03_phase2_postmigration_metrics.md`. Comparisons are all "same bundle, same algorithm, before code change vs after code change". (a) **aa regimes regression guard PASSED at ε = 0** across all 4 (bundle × algorithm) cells × all 3 post-hoc artifacts (12 md5 checks; every byte matched pre-flight). Confirms the pair_key migration is a pure refactor for the aa pipeline at the model-output level. (b) **nt cluster_t099 before/after deltas** — all 4 experiments degrade: HA-NA mild (MLP −1.1 pp F1; LGBM −1.9 pp F1), PB2-PB1 large (MLP −4.8 pp F1; LGBM −13.2 pp F1 / −23 pp MCC). (c) **Triangulation result (Section C of manifest)**: drops are NOT a codebase bug. Script `src/analysis/phase2_routing_triangulation.py` confirmed: (i) positive protein-pair universe byte-identical pre/post (same biology); (ii) DNA-pair universe inflated +34.9 % / +57.0 % per plan §4; (iii) only ~79 % of common positive protein-pairs land in the SAME split pre/post — ~21 % rebinned by LPT-greedy when per-CC pair counts inflated; (iv) 0.03-0.41 % of post protein-pairs have silent DNA variants split across train and val/test (new DNA-variant leakage opened by the migration). Section B metric drops are partly confounded by routing shift, not pure pair_key effect. Mitigation / controlled-experiment options listed in manifest Implications §5. Disentanglement out of scope for Phase 2. Data is gitignored; this commit lands the manifest + plan + memory + triangulation-script updates.
