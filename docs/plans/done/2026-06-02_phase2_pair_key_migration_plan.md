...
**Status: IMPLEMENTED** (pre-flight DONE; commits 1-7 of 7 DONE; HEAD runnable; pair_key semantic change live; test datasets regenerated; models retrained; aa regression guard PASSED at ε = 0 across all 4 cells × 3 post-hoc artifacts; nt cluster before/after deltas measured + triangulated; Key Findings caveats applied to past results)
...
6. **DONE 2026-06-03** — `data(models): retrain MLP + LGBM on test datasets; aa regression guard + nt cluster before/after measurement + routing triangulation`. 12 runs end-to-end (6 MLPs across 2 GPU chains + 6 LGBMs sequential on CPU; ~90 min MLP wall time, ~30 min LGBM). Results manifest at `docs/results/2026-06-03_phase2_postmigration_metrics.md`. Comparisons are all "same bundle, same algorithm, before code change vs after code change". (a) **aa regimes regression guard PASSED at ε = 0** across all 4 (bundle × algorithm) cells × all 3 post-hoc artifacts (12 md5 checks; every byte matched pre-flight). Confirms the pair_key migration is a pure refactor for the aa pipeline at the model-output level. (b) **nt cluster_t099 before/after deltas** — all 4 experiments degrade: HA-NA mild (MLP −1.1 pp F1; LGBM −1.9 pp F1), PB2-PB1 large (MLP −4.8 pp F1; LGBM −13.2 pp F1 / −23 pp MCC). (c) **Triangulation result (Section C of manifest)**: drops are CONSISTENT with #1+#4, no codebase bug evidence — NOT "fully explained". Script `src/analysis/phase2_routing_triangulation.py` confirmed: (i) positive protein-pair universe byte-identical pre/post (same biology); (ii) DNA-pair universe inflated +34.9 % / +57.0 % per plan §4; (iii) only ~79 % of common positive protein-pairs land in the SAME split pre/post — ~21 % rebinned by LPT-greedy when per-CC pair counts inflated; (iv) 0.03-0.41 % of post protein-pairs have silent DNA variants split across train and val/test (new DNA-variant leakage opened by the migration). NOT established: #1/#4 magnitude split, same-data-different-seed variance under the new code, behavioral drift in non-routing code paths. Section B metric drops are partly confounded by routing shift, not pure pair_key effect. Mitigation / controlled-experiment options listed in manifest Implications §5. Disentanglement out of scope for Phase 2. Data is gitignored; this commit lands the manifest + plan + memory + triangulation-script updates.
7. **DONE 2026-06-03** — `docs: Key Findings caveats (pair_key=protein) on past results`. Added header note at top of CLAUDE.md "Key Experimental Findings" stating all entries below were measured under the protein pair_key (pre-Phase-2 convention), pointing forward to the new `pair_key_alphabet` config knob and the post-migration manifest. Inline `(pair_key=protein)` labels added to: 2026-05-15 Experiment B-nt feasibility ceiling (with addendum that the bipartite CC structure / mega-CC fraction is robust to pair_key change), 2026-05-15 "1-NN ≥ LGBM at every cluster_disjoint routing" (with addendum that under nt_cds pair_key the 1-NN advantage would strengthen but isn't cleanly re-testable due to commit-6 routing shift), 2026-05-24 Single-slot HA-only sweep, 2026-05-26 PB2-PB1 single-slot sweep. In `.claude/memory.md`: same `(pair_key=protein)` label + addendum on 2026-05-31 Task 4 multiplicity-skew finding (the protein-level collapse mechanism weakens under nt_cds pair_key; the qualitative claim survives, quantitative bounds shift). No re-running of old experiments per plan § 6. **New experiments under Phase 2 must always cite the alphabet used.**
