...
### 3.2 The cut — port from `bigraph_min_cut`
- Move the recursive-bisection cut into `src/datasets/_megacc_cut.py` (operational home), operating on the production pos_df's `(cluster_a, cluster_b)` edges. The standalone `bigraph_*` analysis scripts stay in `src/analysis` as diagnostics.
...
- **P1 ✓ (2026-06-04)** — `src/datasets/_megacc_cut.py` (ported from `bigraph_min_cut`); unit check reproduced the HA-NA aa t095 result **exactly** (502 pairs / 397 atoms / 7 cuts), row-accounting + determinism confirmed.
