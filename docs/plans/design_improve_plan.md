...
**Status: ✅ IMPLEMENTED** (verified 2026-05-12). `cv_runs/` pattern,
`cv_info.json` per fold, and `src/analysis/compare_cv_runs.py` are all
in place. The full design discussion below is preserved as historical
rationale.
...
---
## Future cleanup (one-line notes — not active work)

These remain non-blocking and unstarted as of 2026-05-12. Captured as
brief notes here instead of full plan sections; promote to a focused
plan if/when one becomes active.

- **Split CV launcher into separate dataset + training scripts.**
  `scripts/run_cv_lambda.py` is currently monolithic; would simplify
  HPC layouts where Stage 3 / 4 run on different machines.
- **CV launcher calls shell wrappers instead of Python directly.**
  Would let `run_cv_*` reuse the `stage3_dataset.sh` / `stage4_train.sh`
  provenance hooks instead of bypassing them with `subprocess.Popen`.
- **Stage 3 pair-generation perf.** Full-dataset (108K isolates) Stage 3
  takes ~11 min vs ~1 min for 5K; bottleneck is per-fold pair
  generation, not I/O or enrichment. Vectorizing the candidate-rejection
  loop and batching negatives are the main wins.
