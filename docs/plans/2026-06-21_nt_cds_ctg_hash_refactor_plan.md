...
- **B1.** [DONE — `ae55f79`+`dd62942`] Build `kmer_features_nt_cds_k3` — extend `compute_kmer_features` with the nt_cds path (reads `cds_dna_final.cds_dna_seq`, keys `brc_fea_id`). `k=6` still pending (B1-k6).
...
- **B4.** nt_ctg feasibility pre-flight (`cluster_disjoint_feasibility` on contig) — establish which thresholds are operable (do NOT assume t100/t099 carries over from nt_cds). *Prereq:* cut `cluster_disjoint_feasibility.py`'s CLI over from `--cds_final`/`--alphabet {aa,nt}` to the registry (`nt_cds`/`nt_ctg`). The metric is the bipartite-component-size distribution, NOT per-function cluster count (the latter is necessary-but-not-sufficient).
...
**B4 (nt_ctg feasibility) — start here:**
- **Prereq:** cut `cluster_disjoint_feasibility.py`'s CLI over from `--cds_final`/`--alphabet {aa,nt}` (the last bare-`'nt'` on the build/analysis path) to the registry (`nt_cds`/`nt_ctg`).
- Run the bipartite-component feasibility on `clusters_nt_ctg` per threshold. **The metric is the largest bipartite-component size vs split capacity (>~80% of pairs → 80/10/10 infeasible), NOT per-function cluster count** (necessary-but-not-sufficient — see the tool's own docstring). Do NOT assume t100/t099 carries over from nt_cds; measure it. Note also the Phase-C experiments use the v2 **2D-CD holdout** (bilateral `cluster_disjoint` + `drop_budget` mega-CC min-cut), which can operate below the strict (no-drop-budget) bin-packing ceiling.
- Then **B1-k6** (heavy nt_cds k6); **Phase C** experiments (C1 baseline needs no new data — nt_cds clusters + nt_ctg features).
