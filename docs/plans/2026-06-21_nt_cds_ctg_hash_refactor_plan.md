...
**Status: Phase A COMPLETE — code cutover + Gate-A regression green (Phase B next).** Done: A1 registry + `tests/test_schema.py`; A2 Stage-1/1.5 producers + non-destructive data migration; the keystone (Stage-3 pair-table → `schema.build_pair_columns()`, all 5 builders) + the `bipartite_components` seq_disjoint fix; all consumers (Stage 4 vocab/bugs, 18 analysis/viz files incl. the `dna_hash` mislabel fix); routing maps → registry (`clustering_utils`/`build_mmseqs_clusters`/`build_cluster_membership`, `compute_seq_hash`→`compute_prot_hash`); k-mer `nt`→`nt_ctg` (builder/loader/Stage 4 + 6 renamed data files + configs). **Gate-A:** Stage-3 dataset ε=0 (byte-identical modulo rename) + Stage-4 `nt_ctg` k-mer training end-to-end verified. Commits: `38fac8e` keystone, `6f5e4cf` consumers, `181c576` routing maps, `591605a` k-mer. Loose ends: `cluster_disjoint_feasibility.py` CLI (`--cds_final`/`--alphabet {aa,nt}`) not yet cut over; `compute_kmer_features` docstring labels; `train_pair_baselines` `kmer_alphabet` provenance. **Phase B** (additive): build `kmer_features_nt_cds`, build `clusters_nt_ctg` + membership, ungate `nt_cds`/`nt_ctg`, nt_ctg feasibility pre-flight. **Phase C:** the 3 experiments (incl. C1 baseline F1≈0.958). Decisions locked (§11).
...
- **B1.** Build `kmer_features_nt_cds_k{3,6}` — extend `compute_kmer_features` with the nt_cds path (reads `cds_dna_final.cds_dna_seq`, keys `brc_fea_id`).
- **B2.** Build `clusters_nt_ctg/` + `cluster_memb_nt_ctg.parquet` (mmseqs contig sweep; ungate `nt_ctg` in `clustering_utils`/`build_mmseqs_clusters`).
- **B3.** Ungate `nt_cds` + `nt_ctg` in `dataset_segment_pairs_v2` (`:1577`, validator `:3015`), `dataset_pairs_cc` (`:369`,`:372`), and the Stage-4 vocab.
- **B4.** nt_ctg feasibility pre-flight (`cluster_disjoint_feasibility` on contig) — establish which thresholds are operable (do NOT assume t100/t099 carries over from nt_cds).

### Phase C — Experiments (v2 2D-CD holdout)
- **C1. Baseline reproduction** — nt_cds clusters + nt_ctg features → reproduce HA/NA t100 F1 ≈ 0.958 (historically `id100`; proves the refactor preserved the known result). Reconstruct the deleted `flu_ha_na_cluster_nt_id100` bundle from the run's `resolved_config.yaml`.
- **C2. Exp 1** — nt_cds clusters + nt_cds features (clean CDS). Isolates the feature axis vs the baseline.
...
## Phase B checkpoint + resume (2026-06-23)

**Phase A: COMPLETE** (code cutover + Gate-A + docs + contract tests). Commits `38fac8e` (keystone) → `ea8d82d` (docs) + `ca4cc16` (tests). Registry `src/utils/schema.py` = single source of truth; 15 contract tests (`test_schema` 10, `test_kmer_utils` 3, `test_clustering_utils` 2).

**Phase B done:**
- v2 `nt_ctg` ungated — `cluster_alphabet ∈ {aa,nt_cds,nt_ctg}`, pos-hash via `schema.hash_col(cluster_alphabet)` (`8580623`).
- nt_cds **cluster** path validated (existing `clusters_nt_cds`): cluster_id cross-split leakage 0/2,044; the small prot_hash overlap is the expected codon-variant behavior.
...
**B2 (nt_ctg clusters) — start here, the meaty remaining piece:**
- `ctg_dna_final` has **no `function` column** (only `canonical_segment`), but `build_mmseqs_clusters`/`export_function_fasta` group by `function`. So nt_ctg clustering needs an input path that **joins `function` onto `ctg_dna_final`** (from `cds_dna_final`/`protein_final` on `(assembly_id, genbank_ctg_id)`) → a `(function, ctg_dna_seq, ctg_dna_hash)` table.
- Ungate `nt_ctg`: `clustering_utils._ACTIVE_ALPHABETS` (L53) + the 4 `NotImplementedError` raises (L100/145/319/439); `build_mmseqs_clusters` raise (L166) + `--alphabet` choices + a `--ctg_dna_final` input mode.
- Then the **heavy mmseqs contig sweep** (easy-linclust over thresholds, 868K contigs) + `build_cluster_membership --alphabet nt_ctg` (its `_ALPHABET_CFG` is already registry-ready).
- nt_ctg UTR caveat stands (exp2 = artifact-included sensitivity check, §9/§12). Then **B4** nt_ctg feasibility pre-flight; **B1-k6** (heavy nt_cds k6); **Phase C** experiments (C1 baseline needs no new data).

**Loose ends (documented):** orchestrator (`dataset_segment_pairs.py`) still reads `cds_final` (works — has `cds_dna_hash` — but should be `cds_dna_final`); `cluster_disjoint_feasibility` CLI (`--cds_final`/`--alphabet {aa,nt}`) not cut over; `compute_kmer_features` docstring labels (`nt:`→`nt_ctg:`); `train_pair_baselines` `kmer_alphabet` provenance (needs param threading); cc nt ungate (not on the experiment path); `compute_kmer_features` `INPUT_BASENAME` is `.csv` (resolves to parquet via `load_dataframe`, but ext-agnostic is cleaner).

**Resume:** `git log --oneline -1` for HEAD; `python tests/test_schema.py tests/test_kmer_utils.py tests/test_clustering_utils.py` to confirm the baseline; tracked tree clean (untracked `reports/` + `cluster_disjoint_ood_audit.py` intentional — never commit the latter).
