...
**B1-k6 — start here:**
- Build `kmer_features_nt_cds_k6` (the k3 path is validated; k6 = 4^6 = 4096 cols, fine for nt). Then **Phase C** experiments: C1 baseline = nt_cds clusters + nt_ctg features (needs no new data, reproduce HA/NA t100 F1≈0.958); exp1 = all nt_cds; exp2 = all nt_ctg (UTR-caveated, §9/§12). Phase-C router = v2 **2D-CD holdout** (bilateral `cluster_disjoint` + `drop_budget`), which can reach below the strict t100/t099 ceiling measured in B4.
