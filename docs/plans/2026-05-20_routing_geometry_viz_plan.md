...
| Routings to compare | `random`, `seq_disjoint`, `cluster_disjoint @ aa id 0.99` | Only `id100` and `id099` are feasible on Flu A under bilateral routing (`splits.md` § 1.9.2); `id099` is the canonical operating point. Skipping a 4th intermediate (id098 already collapses to 93.7%). |
