...
| Routings to compare | `random`, `seq_disjoint`, `cluster_disjoint @ aa id 0.99` | Only `id100` and `id099` are feasible on Flu A (`clustering_overview.md` §9); `id099` is the canonical operating point. Skipping a 4th intermediate (id098 already collapses to 93.7%). |
