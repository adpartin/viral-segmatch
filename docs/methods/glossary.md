...
- **Edge weight** — On the **simple bigraph**, the `weight` of a cluster-pair edge is the **number of positive pairs** carried by that `(cluster_a, cluster_b)` cluster pair — the multiplicity of the bipartite multigraph's parallel edges between those two clusters, collapsed onto the one simple edge (so summing all edge weights recovers the pair count, and a node's ***pair mass*** = the sum of its incident edge weights). It counts **pairs, not sequences** — a weight-`k` edge is one cluster pair carrying `k` co-occurrence examples. It is the NetworkX `weight` edge attribute: set as `weight=len(rows)` in `src/datasets/_megacc_cut.py` (and the analysis `bigraph_*` twins), consumed by the edge min-cut (`nx.fiedler_vector(H, weight='weight')` / Kernighan–Lin; a CC's *size* is `H.subgraph(c).size(weight='weight')`). **Distinct from a *Bridge*** (a topological cut-edge): weight is pair-mass on the edge, a bridge is whether removing the edge disconnects the graph — orthogonal (a bridge may be weight-1 or weight-many). **Cut ⇒ dataset:** dropping a (straddling) edge removes exactly the positive-pair *rows* on that cluster pair from the schema-pair dataset (`kept_pos = pos_with_ids.drop(index=drop_idx)`) — the clusters and their sequences remain (a dropped pair's sequences may persist in other kept pairs); only the **co-occurrence examples** on that cluster pair leave.

- **Connected component (CC)** — A maximal subgraph in which every pair of nodes is path-connected (`nx.connected_components`). In our cluster-level bigraph: a set of HA clusters and NA clusters reachable from one another via alternating HA↔NA edges. The largest CC at low `t` = the **mega-CC**. CC identity is the same on the multigraph and its simple projection. **Two graphs — do not conflate:** unqualified "CC" / "mega-CC" means this **bipartite** cluster-level graph; the connected components of a single-segment **similarity graph** (nodes = sequences; see *Cluster*) are the OOD **clusters** / **mega-cluster** — never call those "CC".
...
- **Cluster** — A group of sequences on one side of the bigraph, at identity threshold `t`, over the per-function **similarity graph** (nodes = unique sequences; an edge = a `≥ t`-identity, `≥ 0.8`-coverage aligned pair). Two build methods on that graph: **set-cover** (`mmseqs easy-linclust` / `easy-cluster --cluster-mode 0`, the default; `build_mmseqs_clusters.py`) or **connected-component** (union-find over the all-vs-all `easy-search` graph — the OOD variant that guarantees "across clusters: different"; validated on M1, `docs/plans/2026-07-08_single_segment_ood_clusters_plan.md`). A cluster is a set of *sequences*, **not** a bipartite CC — never call it "CC" (see *Connected component*).

- **Mega-cluster** (project-specific) — The single giant **cluster** that swallows most of a protein's unique sequences as `t` loosens — the conserved-protein collapse signature (HA0 ~566 aa, length-conserved across subtypes; PB2/PB1 at low `t`; used in `clusters.md`, `splits.md`). The single-segment analog of the **Mega-CC**, but on the per-function *similarity graph* (nodes = sequences), not the bipartite cluster graph. **Do not conflate with Mega-CC:** a mega-cluster is fragmented by a **node/vertex cut** — dropping *bridging sequences* (cost in **nodes**); a mega-CC by an **edge min-cut** — dropping *straddling pairs* (cost in **edges**).
...
- **`m_pos_per_cc`** (config; 2D-CD builder) — Caps the number of positive **pairs (rows) kept per CC** in `dataset_pairs_cc.py` (`split_strategy.m_pos_per_cc`; default 1, `null` = no cap). It changes rows-per-atom, **not** the atom count: atoms (= bipartite CCs) are fixed by the cluster threshold, so only the threshold sets how many there are. `m=1` ⇒ one row per atom (uniform-weight atoms); `m>1` ⇒ up to `m` correlated same-CC rows per atom (non-uniform atoms, so fold sizes vary). Capping never empties a CC, so the atom count is unchanged.

- **Singleton component** — A connected component containing exactly one unique seq pair (one `pair_key` after Mode #1 dedup); equivalently one cluster pair / one bigraph edge. The **negative-infeasible base case**: no within-CC negative can be drawn from it (forming one needs two distinct seq pairs to recombine). `_pair_helpers.bipartite_components` reports the per-graph count as `singleton_components`. Distinct from a *single-isolate* CC (a proxy that misses singletons formed by ≥2 isolates carrying identical sequences) and from a single-*cluster*-pair CC holding multiple seq pairs (not a singleton; it **does** yield negatives).

- **Negative-infeasible CC** — A connected component from which no within-CC negative can be drawn: every recombination of its distinct slot-A × slot-B sequences is a co-occurrence, so every candidate negative reconstructs a true pair. The **singleton component** is the base case (one `pair_key` ⟹ one distinct-a × one distinct-b = the positive); the superset also holds dense CCs that carry ≥2 pair_keys yet have every cross pairing co-occur (e.g. all positives share one slot). Detected structurally and seed-independently (`dataset_pairs_cc.compute_negative_infeasible_ccs`), mirroring the within-CC sampler's feasibility. Dropping these is the `dataset.split_strategy.drop_negative_infeasible_ccs` knob (default true; renamed from `drop_singleton_ccs`), applied identically under both `negative_scope` values so the two scopes route the same CC universe.

- **Within-CC negative** (2D-CD builder) — A negative pair formed inside one bipartite CC: a self isolate's slot-A sequence paired with a *different* partner isolate's slot-B sequence, both drawn from the same CC, rejecting true co-occurrences (Mode #1) and duplicates. Keeps negatives cluster-disjoint by construction (a cross-CC negative would make a cluster span splits). Produced by `src/datasets/_cc_helpers.py`; consumed by the 2D-CD builder (`dataset_pairs_cc.py`).

## Negative-pair sampling

- **Hard negative** (project-specific) — A negative (artificially-mixed, non-co-occurring) segment pair whose two segments share contextual metadata (`host`, `hn_subtype`, `year`) with true positive pairs, so it resembles a positive on demographic-shortcut features and is hard to reject. These are the high-overlap regimes of the 8-regime negative taxonomy (`subtype_year_only`, `host_subtype_year`, …; `src/datasets/_negative_regime_sampling.py`). Raising performance here is the aim of metadata-aware (regime) negative sampling; per-regime performance is reported in `level1_neg_regimes.png` (`src/analysis/analyze_stage4_train.py`). Related: **demographic shortcut leakage** (`docs/plans/2026-05-07_leakage_diagnostics_plan.md`).

- **Easy negative** (project-specific) — A negative segment pair whose two segments share no contextual metadata (the `none_match` regime in the 8-regime negative taxonomy), so they are easily separable via a demographic shortcut. Easy negatives dominate a regime-blind negative set, so aggregate metrics (F1, etc.) can improve while hard-negative performance stays poor — the masking effect documented in `docs/results/2026-05-07_metadata_shortcut_negatives.md`.
