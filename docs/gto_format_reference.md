...
**Scope.** Hand-authored reference for the Genome Typed Object (GTO) JSON
files that feed Stage 1 of this pipeline. Focuses on **Influenza A** since
that is the active subject of the project; Bunyavirales GTOs use the same
container but are not currently maintained (see CLAUDE.md → "What Is NOT
Maintained").

**Companion files.**
- `docs/examples/example_gto_excerpt.json` — a trimmed, real GTO (assembly
  `0000003012`, A/mallard/Wisconsin/394/1977 H4N1) showing every field
...
- `src/preprocess/preprocess_flu.py` — the only code in this repo that
  reads GTO JSON directly. Section "What Stage 1 parses" below has
  file:line pointers.
...
2. **Contigs** — the raw nucleotide sequences of each segment, with
   minimal per-contig metadata (which segment, geometry, contig-level
   quality).
...
Crucially, **the nucleotide layer and the protein/annotation layer are
...
platform default, not a biological claim. The codes share the
canonical start (ATG) and stops (TAA / TAG / TGA), so the discrepancy
has no consequence for any operation we currently perform — but
anyone doing codon-level work downstream should treat this field as
non-authoritative for flu and hard-code code 1 instead.
...
2. **`start_1based`** (str or int) — the **1-based** start position on the
   plus strand of the contig. (Strings and ints both occur in real data;
   downstream code casts.)
...
4. **`length`** (str or int) — span **length in nucleotides**, *not* an
   end coordinate. End position = `start + length - 1` (inclusive).

### 5.1 Single-entry case (unspliced)

Most flu CDSs are unspliced. Example PB2 in the excerpt:
...
Flu has two spliced gene products: **M2** (on Segment 7) and **NEP**
(formerly NS2, on Segment 8). Both arise from alternative splicing of an
mRNA, so their CDSs are non-contiguous on the parent contig. Example M2
from the excerpt:
...
- **Coding region (CDS)** — in this corpus: starts at ATG in 99.66% of
  sampled major CDSs, ends at a canonical stop (TAA / TAG / TGA) in
  99.02%, and the annotated `location` length is a multiple of 3 in
...
- **Introns** — for spliced features (M2, NEP), the nucleotides in the
  ...
  as its own feature.
- **Overlapping ORFs** — for PB1 (PB1 + PB1-F2 + PB1-N40) and PA
  (PA + PA-X + PA-N155 + PA-N182), the contig nucleotides are shared
  across multiple `features[*]` entries. There is no "ORF map" beyond
...
| S7 (M)   | 1.63% | 1.11% | (−0.11%) | 26.10% |
| S8 (NS)  | 2.73% | 3.01% | (−0.12%) | 26.18% |
...
**Contig lengths per canonical segment (nucleotides):**

| Segment | min | median | max | spread |
|---|---:|---:|---:|---:|
| S1 | 2201 | 2296 | 2417 | 216 |
| S2 | 2201 | 2301 | 2379 | 178 |
| S3 | 2150 | 2189 | 2266 | 116 |
| S4 | 1650 | 1715 | 1817 | 167 |
| S5 | 1460 | 1524 | 1596 | 136 |
| S6 | 1338 | 1420 | 1498 | 160 |
| S7 |  906 |  993 | 1069 | 163 |
| S8 |  774 |  860 |  928 | 154 |

(All `n = 108,530`; counted from `length` column in `genome_final.parquet`.)

**Comparison.** Pairing each segment with its primary CDS:
PB2 contig range 216 vs CDS range 87; PB1 178 vs 99; PA 116 vs 90;
