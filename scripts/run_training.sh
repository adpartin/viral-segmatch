...
# Usage: ./scripts/run_training.sh <config_bundle> [--cuda_name CUDA] [--dataset_dir DIR] [--embeddings_file FILE] [--output_dir DIR]
# Example: ./scripts/run_training.sh flu_a_learning_test --cuda_name cuda:0
#          ./scripts/run_training.sh flu_a_overfit_test --cuda_name cuda:1 --dataset_dir /custom/path
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    if [ "$SKIP_POSTPROCESSING" = false ]; then
        ...
        # Determine output directory (use override or auto-detect from config)
        if [ -z "$OUTPUT_DIR" ]; then
            # Auto-detect from config (Python script will handle this)
            ACTUAL_OUTPUT_DIR=""
        else
        ...
        POSTPROC_CMD="python $PROJECT_ROOT/src/postprocess/segment_classifier_results.py --config_bundle $CONFIG_BUNDLE"
        ...
        PRESENTATION_CMD="python $PROJECT_ROOT/src/postprocess/presentation_plots.py --config_bundle $CONFIG_BUNDLE"
        if [ -n "$ACTUAL_OUTPUT_DIR" ]; then
