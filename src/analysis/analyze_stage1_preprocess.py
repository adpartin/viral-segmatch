"""
Analyze preprocessing results (Stage 1) for viral protein data.
...
Plots generated:
- data_count_flow_analysis.png: Records and unique proteins through pipeline stages
- segment_distribution.png: Overall segment distribution (bar chart)
- core_protein_assignment.png: Core protein segment assignment (stacked bar)
- auxiliary_protein_assignment.png: Auxiliary protein segment assignment (stacked bar)
- replicon_assignment_rate.png: Assignment rate by replicon type
- sequence_length_distribution.png: Sequence length histogram by segment
- sequence_length_by_segment.png: Sequence length boxplots by segment
- function_distribution.png: Top 10 protein functions (horizontal bar)
- quality_distribution_by_segment.png: Quality distribution by segment (stacked bar)

Other outputs:
- preprocessing_flow_metrics.csv: Stage-by-stage metrics table
- preprocessing_metrics.json: Detailed metrics in JSON format
- preprocessing_analysis_report.txt: Comprehensive text report

Usage:
    python src/analysis/analyze_stage1_preprocess.py --virus bunya --data_version April_2025
    python src/analysis/analyze_stage1_preprocess.py --virus flu_a --data_version July_2025
"""
...
def get_segment_config(virus_name: str) -> tuple:
    ...
    if virus_name == 'bunya':
        ...
        # Flu A has 8 segments (1-8)
        tab10 = plt.cm.tab10.colors
        colors = {str(i): tab10[i-1] for i in range(1, 9)}
...
def get_protein_functions(virus_name: str, data_version: str) -> tuple:
    """Get core and auxiliary protein functions for a virus."""
    if virus_name == 'bunya':
        ...
        # Flu A core proteins (8 segments - one per segment)
        core = [
...
def load_preprocessing_data():
    ...
    data_files = {
        'initial': ['protein_agg_from_GTOs.csv', 'protein_agg_from_GTOs.parquet'],
        'after_segment_assignment': ['protein_assigned_segments.csv', 'protein_assigned_segments.parquet'],
        'after_basic_filtering': ['protein_filtered_basic.csv', 'protein_filtered_basic.parquet'],
        'final': ['protein_final.csv', 'protein_final.parquet']
    }

    data = {}
    for stage, fnames in data_files.items():
        loaded = False
        for fname in fnames:
            fpath = processed_data_dir / fname
            if fpath.exists():
                if fname.endswith('.parquet'):
                    data[stage] = pd.read_parquet(fpath)
                else:
                ...
                break
        if not loaded:
            print(f'Warning: {fnames[0]} not found (tried csv and parquet)')
...
def _create_separate_segment_figures(df):
    ...
    if not core_df.empty:
        core_segment_func = pd.crosstab(core_df['function'], core_df['canonical_segment'])
        if not core_segment_func.empty:
            # Map protein names to shorter versions
            core_segment_func.index = [map_protein_name(name) for name in core_segment_func.index]
            ...
            ax2.text(0.5, 0.5, 'No core protein data available', 
                    ha='center', va='center', transform=ax2.transAxes, fontsize=12)
            ax2.set_title('Core Protein Segment Assignment', fontsize=14, fontweight='bold')
    else:
        ax2.text(0.5, 0.5, 'No core proteins found in data', 
                ha='center', va='center', transform=ax2.transAxes, fontsize=12)
        ax2.set_title('Core Protein Segment Assignment', fontsize=14, fontweight='bold')
...
def main(virus_name: str, data_version: str):
    """Main analysis function."""
    
    # Set up paths and config based on args
    ...
    # Get virus-specific config
    segment_colors, segment_order = get_segment_config(virus_name)
    core_functions, aux_functions = get_protein_functions(virus_name, data_version)
    
    # Make these available globally for the analysis functions
    globals()['processed_data_dir'] = processed_data_dir
    globals()['results_dir'] = results_dir
    globals()['SEGMENT_COLORS'] = segment_colors
    globals()['SEGMENT_ORDER'] = segment_order
    globals()['core_functions'] = core_functions
    globals()['aux_functions'] = aux_functions
    globals()['DATA_VERSION'] = data_version

    # Apply default plotting style
    apply_default_style()

    print(f"\n{'='*60}")
    print(f'Stage 1: Preprocessing Analysis')
    print(f"{'='*60}")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Analyze Stage 1: preprocessing results')
