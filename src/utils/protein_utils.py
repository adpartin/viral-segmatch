...
# =============================================================================
# CORE PROTEIN FILTERING AND VALIDATION
# =============================================================================

def get_core_protein_filter_mask(df):
    """
    Create a boolean mask to filter dataframe to core proteins only.
    
    TODO: Move the segment-function validation to preprocessing stage.
    The filtering step should assume data is already biologically consistent.
    
    Args:
        df: DataFrame with 'canonical_segment' and 'function' columns
        
    Returns:
        Boolean mask for core proteins
    """
    from src.utils.config import USE_CORE_PROTEINS_ONLY
    
    if not USE_CORE_PROTEINS_ONLY:
        return pd.Series([True] * len(df))
    
    # Core segment-function mapping for biological validation
    # TODO: Move this to preprocessing stage
    CORE_SEGMENT_FUNCTION_MAPPING = {
        'L': 'RNA-dependent RNA polymerase',
        'M': 'Pre-glycoprotein polyprotein GP complex',
        'S': 'Nucleocapsid protein'
    }
    
    # For now, validate both segment and function consistency (original logic)
    # This will be moved to preprocessing in the future
    mask = (
        df['canonical_segment'].isin(CORE_SEGMENT_FUNCTION_MAPPING.keys()) &
        df.apply(lambda row: row['function'] == CORE_SEGMENT_FUNCTION_MAPPING.get(row['canonical_segment']), axis=1)
    )
    return mask


def validate_core_proteins(df):
    """
    Validate that core protein constraints are met.
    
    TODO: Move this validation to preprocessing stage (preprocess_bunya_protein.py).
    The dataset creation step should assume data is already validated.
    
    Args:
        df: DataFrame with protein data (should already be filtered to core proteins)
    ...
    # Check for isolates with too many core proteins
    for assembly_id, group in df.groupby('assembly_id'):
        n_proteins = len(group)
        if n_proteins > 3:
            functions = group['function'].tolist()
            segments = group['canonical_segment'].tolist() if 'canonical_segment' in group.columns else ['N/A'] * len(functions)
            raise ValueError(
                f"Assembly {assembly_id} has {n_proteins} core proteins (expected ≤3). "
                f"Functions: {functions}, Segments: {segments}"
            )