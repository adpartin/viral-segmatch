...
- Regime-aware leaves: `flu_ha_na_neg_regimes`, `flu_pb2_pb1_neg_regimes` (2026-05-10) -- inherit from `flu_ha_na` / `flu_pb2_pb1` and add `dataset.negative_sampling.regime_targets` + `baselines: {enabled: [...]}`. Both bundles now use the uniform `0.10 x 7 + 0.30 x 1` regime mix (`host_subtype_year` keeps the 0.30 weight; the other 7 regimes are uniform at 0.10) after the pb2_pb1 flatten on 2026-05-11.
- Mode #3 mitigation leaf: `flu_ha_na_seq_disjoint` (2026-05-11) -- inherits `flu_ha_na_neg_regimes` with `dataset.split_strategy.mode=seq_disjoint`.
- Metadata-holdout demo: `flu_ha_na_metadata_holdout_temporal` (2026-05-11) -- inherits `flu_ha_na_neg_regimes` with year-axis `metadata_holdout` (train 2021-2023, test 2024). Year-axis is the degenerate case of the general mechanism; multi-axis filters are first-class.
...
- seq_disjoint split mode (2026-05-11): `dataset.split_strategy.mode={random,seq_disjoint}` in v2 builder. Bipartite-CC LPT-greedy on (HA-DNA, NA-DNA) graph; whole components are indivisible so cross-split DNA leakage is impossible by construction (and zero pair drops on Flu A HA/NA: 46,954 components, largest 178 pairs / 0.30%). The v2 negative sampler operates entirely on per-split `pos_df`, so per-split negatives inherit the seq_disjoint property automatically — no extra plumbing. Writes `seq_disjoint_audit.json` (re-checked on full pairs incl. negatives; hard-fail in saver on any non-zero overlap). Bundle: `conf/bundles/flu_ha_na_seq_disjoint.yaml`. Single-split only — CV combo is rejected loudly. Plan: `docs/plans/done/2026-05-10_seq_disjoint_routing_plan.md`. Results: `docs/results/2026-05-11_exp4a_seq_disjoint_results.md`.
...
- Methods docs added (2026-05-10):
  ...
  - `docs/methods/pipeline_overview.md` -- multi-audience synthesis (biologists, bioinformaticians, data scientists). Task framing → reassortment biology → 4 stages → filtering order → pair construction (positive dedup, negative coverage, the 8 regimes) → train/val/test balance impact → features → models → heatmap interpretation → 5-mode leakage map. NOTE: doc body still mentions 9 regimes from the original 2026-05-10 write-up; needs a refresh pass to align with the 2026-05-11 8-regime taxonomy.
  - `docs/post_hoc_analysis_design.md` -- updated Level 1 section + outputs to the new schema (8-regime + agg). NOTE: same pending refresh as pipeline_overview.md; the table on line 102 still references `unknown_metadata_neg`.
...
- **Mode #3 (sequence-level leakage) — IMPLEMENTED 2026-05-11** via seq_disjoint routing. Bipartite-CC LPT-greedy on (HA-DNA, NA-DNA); cross-split dna_hash overlap = 0 with zero pair drops on HA/NA. Headline finding: MLP host_subtype_year TNR drops 0.872 → 0.834; MLP now **below** 1-NN on that regime (gap −0.014). Baselines barely move. See `docs/results/2026-05-11_exp4a_seq_disjoint_results.md` and `docs/plans/done/2026-05-10_seq_disjoint_routing_plan.md`. strict_dedup deliberately deferred (seq_disjoint achieves the same test with no data loss). Cluster-level leakage (mode #4) remains open — see `docs/plans/2026-05-08_cosine_and_cluster_splits_plan.md`.
