...
- **MMD on PCA-50 ESM-2 + aa k=3 per-slot DID discriminate routings at 1000-isolate scale on Flu A HA/NA (2026-05-24)**: script `src/analysis/mmd_per_slot.py` in `segmatch` env. Phase 1 wiring sanity passed on both feature spaces (10 random per-entity 50/50 splits give MMD² at the bias-term noise floor). Phase 2 with fixed σ per feature space (ESM-2 σ = 1.0719, aa k=3 σ = 29.3192; values differ because k-mer count vectors have larger raw magnitudes than ESM-2 embeddings → cross-feature-space MMD² values NOT directly comparable, ratios + p-values are) and 500-permutation p-values, run on the same 3 datasets as the L(π) work: cluster_disjoint id099 reaches p ≈ 0.002 (= 1/501 floor; 0/500 perms exceeded observed) in all four (slot × feature space) cells — robust to both. random non-significant in all four cells (p > 0.6). seq_disjoint feature-space-dependent on HA (ESM-2 p = 0.166 not sig; aa k=3 p = 0.004 sig) and consistent on NA (both p ≈ 0.06 borderline). HA seq_disjoint divergence consistent with project's "K-mer dominates ESM-2 on H3N2" finding but slot-dependent — not a universal aa-k3 advantage. No HA/NA asymmetry analogous to the L(π) finding. Caveat on absolute MMD² interpretability: kernel-dependent, not meaningful in isolation — defensible framing is within-feature-space ratio to random baseline + permutation p-value. Not tested: full-corpus scale, pair-level (S2), nt k-mer features, downstream generalization gap. Full writeup: `docs/results/2026-05-24_mmd_per_slot_results.md`. Plan status: `docs/plans/2026-05-22_split_separation_metrics_plan.md` (MMD leg PHASE 1+2 DONE, full-corpus follow-up pending).
