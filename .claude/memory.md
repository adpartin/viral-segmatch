...
- **MMD on PCA-50 ESM-2 per-slot DID discriminate routings at 1000-isolate scale on Flu A HA/NA (2026-05-24)**: script `src/analysis/mmd_per_slot.py` in `segmatch` env. Phase 1 wiring sanity passed (10 random per-entity 50/50 splits on HA give MMD² ≈ 0.002 ± 0.0015, consistent with the biased estimator's O(1/N) finite-sample bias). Phase 2 on the same three datasets as the L(π) work, with fixed σ = 1.0719 and 500-permutation p-values, produced the ordering random ≤ seq_disjoint < cluster_disjoint id099 on both slots. cluster_disjoint id099 reached p ≈ 0.002 (= 1/501, the add-one-smoothed minimum; 0/500 permutations exceeded observed) on both slots: HA MMD² = +0.042, NA MMD² = +0.015. seq_disjoint borderline (HA p = 0.166, NA p = 0.056). random not significant (HA p = 0.639, NA p = 0.864). HA and NA agree on ordering — no HA/NA asymmetry analogous to the L(π) finding. HA's absolute MMD² values are roughly 3× NA's at the fixed σ; not disentangled (could be biology or σ-tuning since 1.0719 was median heuristic on HA's PCA-50). Not tested: full-corpus scale, pair-level (S2), k-mer features, downstream generalization gap. Caveat on absolute MMD² interpretability: the value is kernel-dependent and not meaningful in isolation — the defensible framing is the ratio to the random baseline plus the permutation p-value. Full writeup: `docs/results/2026-05-24_mmd_per_slot_results.md`. Plan status: `docs/plans/2026-05-22_split_separation_metrics_plan.md` (MMD leg PHASE 1+2 DONE, full-corpus follow-up pending).
